...
export async function generateMetadata({
  params,
}: Fc01vProductPageProps): Promise<Metadata> {
...
export default async function Fc01vProductPage({ params, searchParams }: Fc01vProductPageProps) {
  const { slug } = await params;
  const query = await searchParams;

  return <PdpProductPageView slug={slug} searchParams={query} version="fc01v" />;
}
