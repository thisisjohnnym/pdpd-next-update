...
export async function generateMetadata({
  params,
}: V8ProductPageProps): Promise<Metadata> {
...
export default async function V8ProductPage({ params, searchParams }: V8ProductPageProps) {
  const { slug } = await params;
  const query = await searchParams;

  return <PdpProductPageView slug={slug} searchParams={query} version="v8" />;
}
