...
export async function generateMetadata({
  params,
}: V5ProductPageProps): Promise<Metadata> {
...
export default async function V5ProductPage({ params, searchParams }: V5ProductPageProps) {
  const { slug } = await params;
  const query = await searchParams;

  return <PdpProductPageView slug={slug} searchParams={query} version="v5" />;
}
