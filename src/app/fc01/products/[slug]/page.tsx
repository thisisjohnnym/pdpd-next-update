...
export async function generateMetadata({
  params,
}: Fc01ProductPageProps): Promise<Metadata> {
...
export default async function Fc01ProductPage({ params, searchParams }: Fc01ProductPageProps) {
  const { slug } = await params;
  const query = await searchParams;

  return <PdpProductPageView slug={slug} searchParams={query} version="fc01" />;
}
