...
export async function generateMetadata({
  params,
}: V6ProductPageProps): Promise<Metadata> {
...
export default async function V6ProductPage({ params, searchParams }: V6ProductPageProps) {
  const { slug } = await params;
  const query = await searchParams;

  return <PdpProductPageView slug={slug} searchParams={query} version="v6" />;
}
