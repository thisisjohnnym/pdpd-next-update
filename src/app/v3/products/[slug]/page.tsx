...
export async function generateMetadata({
  params,
}: V3ProductPageProps): Promise<Metadata> {
...
export default async function V3ProductPage({ params, searchParams }: V3ProductPageProps) {
  const { slug } = await params;
  const query = await searchParams;

  return <PdpProductPageView slug={slug} searchParams={query} version="v3" />;
}
