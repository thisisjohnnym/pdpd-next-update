...
export async function generateMetadata({
  params,
}: V2ProductPageProps): Promise<Metadata> {
...
export default async function V2ProductPage({ params, searchParams }: V2ProductPageProps) {
  const { slug } = await params;
  const query = await searchParams;

  return <PdpProductPageView slug={slug} searchParams={query} version="v2" />;
}
