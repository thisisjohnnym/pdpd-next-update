...
export async function generateMetadata({
  params,
}: V4ProductPageProps): Promise<Metadata> {
...
export default async function V4ProductPage({ params, searchParams }: V4ProductPageProps) {
  const { slug } = await params;
  const query = await searchParams;

  return <PdpProductPageView slug={slug} searchParams={query} version="v4" />;
}
