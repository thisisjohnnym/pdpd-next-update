...
export async function generateMetadata({
  params,
}: V7ProductPageProps): Promise<Metadata> {
...
export default async function V7ProductPage({ params, searchParams }: V7ProductPageProps) {
  const { slug } = await params;
  const query = await searchParams;

  return <PdpProductPageView slug={slug} searchParams={query} version="v7" />;
}
